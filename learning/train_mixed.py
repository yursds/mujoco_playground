...
def train(
  ...
  # Track current model distribution for real-time counting
  ...
  def training_step(
    ...
    def f(carry, unused_t):
      ...
      # Use original acting.generate_unroll but with action tail freezing
      def generate_unroll_with_frozen_tail(env, env_state, policy, key, unroll_length, extra_fields=()):
        """Generate unroll with frozen action tail for model selection."""
        
        # Get the first action to extract the frozen tail
        first_actions, _ = policy(env_state.obs, key)
        
        # Determine if we have a tail and what the frozen tail should be
        try:
          base_size = getattr(env.unwrapped, 'action_size')
        except Exception:
          base_size = getattr(getattr(env.unwrapped, 'mj_model', None), 'nu', first_actions.shape[-1])
        
        has_tail = first_actions.shape[-1] > int(base_size)
        frozen_tail = first_actions[..., base_size:] if has_tail else None
        
        @jax.jit
        def f_unroll(carry, unused_t):
          state, current_key = carry
          current_key, next_key = jax.random.split(current_key)
          
          # Get policy action
          actions, policy_extras = policy(state.obs, current_key)
          
          # Replace tail with frozen tail if we have one
          if has_tail and frozen_tail is not None:
            base_action = actions[..., :base_size]
            actions = jnp.concatenate([base_action, frozen_tail], axis=-1)
        ...
        (final_state, _), data = jax.lax.scan(
            f_unroll, (env_state, key), (), length=unroll_length
        )
        return final_state, data
      
      next_state, data = generate_unroll_with_frozen_tail(
          env,
          current_state,
          policy,
          current_key,
          unroll_length,
          extra_fields=('truncation', 'episode_metrics', 'episode_done', 'active_model'),
      )
    ...
    # Always update model distribution if active_model is available (for WandB tracking)
    
    if ('state_extras' in data.extras and 
        'active_model' in data.extras['state_extras'] and 
        data.extras['state_extras']['active_model'] is not None):
      # Get model indices from all steps and flatten to get all environments
      # data.extras['state_extras']['active_model'] has shape (batch_size * num_minibatches, unroll_length)
      # We want the latest step for each environment
      model_indices = data.extras['state_extras']['active_model'][:, -1]  # Shape: (batch_size * num_minibatches,)

      def update_model_distribution_local(model_indices):
        # Reset counts
        nonlocal current_model_counts
        current_model_counts.clear()
        
        # Count environments per model
        if model_indices is not None:
          model_indices_np = np.array(model_indices).flatten()
          unique_indices, counts = np.unique(model_indices_np, return_counts=True)
          
          for idx, count in zip(unique_indices, counts):
            current_model_counts[f"model_{int(idx)}_env_count"] = int(count)
          
      
      jax.debug.callback(
          update_model_distribution_local,
          model_indices,
      )

    # Count how many actions contained an appended model-selection tail and
    # log it. `has_tail` is a boolean included in Transition.extras for each
    # unroll step; we sum over unroll and batch dims to get a single count.
    def _log_tail_stats(has_tail_array):
      # has_tail_array shape: (batch, unroll_length)
      total = int(jnp.sum(has_tail_array))
      logging.info('Rollout contained %d actions with selection tail in this batch', total)
  ...
  # Try to get number of models from environment (unwrapped if needed)
  num_models = 2  # default
  try:
    num_models = getattr(env, 'num_models', None)
    if num_models is None:
      num_models = getattr(env.unwrapped, 'num_models', 2)
  except:
    num_models = 2
  ...
  def training_epoch_with_timing(
    ...
    nonlocal training_walltime, num_models, current_model_counts
    t = time.time()
    ...
    # Always send current model distribution to WandB (regardless of completed episodes)
    model_distribution_metrics = {}
    if current_model_counts:
      total_envs = sum(current_model_counts.values())
      for i in range(num_models):
        env_count_key = f"model_{i}_env_count"
        env_count = current_model_counts.get(env_count_key, 0)
        percentage = (env_count / total_envs) * 100 if total_envs > 0 else 0
        model_distribution_metrics[f"model_distribution/model_{i}_count"] = env_count
        model_distribution_metrics[f"model_distribution/model_{i}_percentage"] = percentage
      model_distribution_metrics["model_distribution/total_envs"] = total_envs
    
    metrics = {
        'training/sps': sps,
        'training/walltime': training_walltime,
        **{f'training/{name}': value for name, value in metrics.items()},
        **model_distribution_metrics,  # Always include model distribution
    }
