...
def configure_mixed_training(
  ...
  train_fn = functools.partial(
      ...
      **training_params,
      ...
      wrap_env_fn=None if _VISION.value else partial_wrap_fn, # ! ksssfvoufoufvsohihsodi
      num_eval_envs=num_eval_envs,
  )

  return train_fn, training_params

def run_evaluation_and_render_mixed(make_inference_fn, params, env_cfg):
  ...
  # !!!! =============================================================
  from mujoco_playground._src.locomotion_mixed.kawaru.joystick import Joystick
  
  # Create a list of single, unwrapped environments to render
  render_envs = []
  for path in env_cfg.models_config.xml_paths:
      mj_model = mujoco.MjModel.from_xml_path(path)
      render_env = Joystick(config=env_cfg, mj_model=mj_model)
      render_envs.append(render_env)

  for i, render_env in enumerate(render_envs):
    print(f"Rendering model {i+1}/{len(render_envs)}")
    
    # Run a single rollout for this specific environment
    jit_reset = jax.jit(render_env.reset)
    jit_step = jax.jit(render_env.step)
    rng = jax.random.PRNGKey(123)
    rng, reset_rng = jax.random.split(rng)
    
    # We pass the specific MJX model to the reset function
    # Note: the `Joystick` reset method has an extra `mjx_model` argument.
    ...
    for _ in range(env_cfg.episode_length):
      ...
      # We pass the specific MJX model to the step function
      state = jit_step(state, ctrl)
      rollout.append(state)
      if state.done:
        break

    # Render the trajectory and save the video
    render_every = 2
    fps = 1.0 / render_env.dt / render_every
    ...
    media.write_video(f"rollout_model_{i}.mp4", frames, fps=fps)
    print(f"Rollout video for model {i+1} saved as 'rollout_model_{i}.mp4'.")
...
def main(argv):
  ...
  run_evaluation_and_render_mixed(make_inference_fn, params, env_cfg)
